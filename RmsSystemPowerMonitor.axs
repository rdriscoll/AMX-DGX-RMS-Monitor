...
DEFINE_FUNCTION RmsEventAssetRegistered(CHAR assetClientKey[], LONG assetId, CHAR newAssetRegistration)
{
  ...
  IF(assetClientKey == RmsDevToString(dvControlSystem))
  {
...
DEFINE_FUNCTION RmsEventSystemPowerChanged(CHAR powerOn)
{
  ...
  {
    // system power in ON
    RmsAssetParameterSetValue(RmsDevToString(dvControlSystem),'system.power','On');
  }
  ELSE
  {
    // system power in OFF
    RmsAssetParameterSetValue(RmsDevToString(dvControlSystem),'system.power','Off');
  }
}
